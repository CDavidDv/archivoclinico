...
<div class="container">
    ...
    <h1 class="mb-4 text-success fw-bold">Entradas de Farmacia</h1>

    <div class="card shadow border-0">

        <div class="card-header bg-success text-white d-flex justify-content-between align-items-center">
            <span class="fw-semibold">Historial de Entradas</span>
            <a href="{{ route('entradas_farmacia.create') }}" class="btn btn-light btn-sm shadow-sm">
                + Nueva Entrada (ajuste)
            </a>
        </div>
                    ...
                    <tbody>
                        @forelse($entradas as $entrada)
                            <tr>
                                ...
                                <td>{{ $entrada->transferencia?->folio ?? '—' }}</td>
                                <td>{{ $entrada->detalles_count }}</td>
                                <td>{{ $entrada->usuario?->nombre_usuario }}</td>
                                <td>
                                    <a href="{{ route('entradas_farmacia.show', $entrada) }}"
