...
<template>
    <AppLayout title="Detalle de Entrada">
        <PageHeader :title="`Entrada #${entrada.id}`">
            <template #actions>
                <Link :href="route('entradas_almacen.index')" class="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Volver</Link>
            </template>
        </PageHeader>
        ...
        <Card title="Datos de la Entrada" class="mb-6">
            <dl class="grid gap-x-6 gap-y-3 sm:grid-cols-2 text-sm">
                ...
                <div><dt class="text-gray-500">Tipo</dt><dd class="font-medium capitalize">{{ entrada.tipo }}</dd></div>
                <div><dt class="text-gray-500">Proveedor</dt><dd class="font-medium">{{ entrada.proveedor ? entrada.proveedor.nombre : '—' }}</dd></div>
        ...
        <Card title="Productos ingresados">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                    <thead>
                        <tr class="text-left text-gray-500">
                            ...
                            <th class="px-3 py-2 font-medium">Precio unit.</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        <tr v-for="d in entrada.detalles" :key="d.id">
                            ...
                            <td class="px-3 py-2">{{ d.precio_unitario ?? '—' }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
