                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        <tr v-for="e in entradas.data" :key="e.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                            ...
                            <td class="px-3 py-2">{{ e.proveedor ? e.proveedor.nombre : '—' }}</td>
                            <td class="px-3 py-2">{{ e.usuario ? e.usuario.nombre_usuario : '—' }}</td>
                            <td class="px-3 py-2">{{ e.detalles_count }}</td>
                            <td class="px-3 py-2 text-right">
                                <Link :href="route('entradas_almacen.show', e.id)" class="rounded border border-emerald-500 px-2 py-1 text-xs text-emerald-700 hover:bg-emerald-50">Ver</Link>
                            </td>
                        </tr>
                        <tr v-if="entradas.data.length === 0"><td colspan="7" class="px-3 py-6 text-center text-gray-400">No hay entradas.</td></tr>
                    </tbody>
                </table>
            </div>
