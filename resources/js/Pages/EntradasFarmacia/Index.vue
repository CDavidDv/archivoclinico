...
<template>
    <AppLayout title="Entradas de Farmacia">
        <PageHeader title="Entradas de Farmacia">
            <template #actions>
                <Link :href="route('entradas_farmacia.create')" class="rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700">+ Nueva Entrada</Link>
            </template>
        </PageHeader>

        <Card title="Historial de Entradas">
            <DataTable :columns="columns" :paginator="entradas" route-name="entradas_farmacia.index" :filters="filtros" :orden="orden" has-actions empty="No hay entradas.">
