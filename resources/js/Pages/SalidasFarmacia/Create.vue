...
const props = defineProps({ lotes: Array, tipos: Array });

const fecha = (f) => (f ? new Date(f).toLocaleDateString('es-MX') : '—');
const loteOptions = computed(() => props.lotes.map((l) => ({
    value: l.id,
    label: `${l.medicamento ? l.medicamento.clave : ''} ${l.medicamento ? l.medicamento.nombre : ''} · Lote ${l.numero_lote} · vence ${fecha(l.caducidad)} · disp ${l.cantidad_actual}`,
})));
const tipoOptions = computed(() => props.tipos.map((t) => ({ value: t, label: t.charAt(0).toUpperCase() + t.slice(1) })));

const form = useForm({
    tipo: '', fecha: new Date().toISOString().slice(0, 10), observaciones: '',
    detalles: [{ id_lote_farmacia: '', cantidad: 1 }],
});

const agregar = () => form.detalles.push({ id_lote_farmacia: '', cantidad: 1 });
const quitar = (i) => form.detalles.length > 1 && form.detalles.splice(i, 1);
const submit = () => form.post(route('salidas_farmacia.store'));
</script>

<template>
    <AppLayout title="Nueva Salida de Farmacia">
        <PageHeader title="Nueva Salida de Farmacia" />
        <Card title="Registro de Salida">
            <form @submit.prevent="submit">
                <div class="grid gap-4 sm:grid-cols-2">
                    <SelectField v-model="form.tipo" label="Tipo de salida" :options="tipoOptions" placeholder="Seleccione tipo" :error="form.errors.tipo" required />
                ...
                <h3 class="mt-6 mb-2 text-sm font-semibold text-gray-700">Lotes afectados</h3>
                ...
                <div v-for="(d, i) in form.detalles" :key="i" class="mb-3 grid grid-cols-12 gap-2">
                    <div class="col-span-8"><SelectField v-model="d.id_lote_farmacia" :options="loteOptions" placeholder="Lote" :error="form.errors[`detalles.${i}.id_lote_farmacia`]" /></div>
                ...
                <div class="mt-2 flex gap-2">
                    ...
                    <Link :href="route('salidas_farmacia.index')" class="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Cancelar</Link>
                </div>
            </form>
        </Card>
    </AppLayout>
