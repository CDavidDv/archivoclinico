...
<template>
    <AppLayout title="Detalle de Salida">
        <PageHeader :title="`Salida #${salida.id}`">
            <template #actions>
                <Link :href="route('salidas_farmacia.index')" class="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Volver</Link>
            </template>
        </PageHeader>
        ...
        <Card title="Datos de la Salida" class="mb-6">
            <dl class="grid gap-x-6 gap-y-3 sm:grid-cols-2 text-sm">
                ...
                <div><dt class="text-gray-500">Tipo</dt><dd class="font-medium capitalize">{{ salida.tipo }}</dd></div>
